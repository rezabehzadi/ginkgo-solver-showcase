...
int main()
{
    ...
    auto M = gko::matrix::Dense<double>::create(exec, gko::dim<2>{n, n});
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            M->at(i, j) = dist(gen);
        }
    ...
    // Compute A = M^T * M + alpha * I to get SPD matrix
    double alpha = 1e-3;  // small regularization term

    for (int i = 0; i < n; ++i) {
        for (int j = 0; j <= i; ++j) {  // compute lower triangle and copy to upper later
            double sum = 0.0;
            for (int k = 0; k < n; ++k) {
                sum += M->at(k, i) * M->at(k, j);
            }
            A_unique->at(i, j) = sum;
            A_unique->at(j, i) = sum;  // symmetry
        }
        A_unique->at(i, i) += alpha;  // add regularization to diagonal
    }
    ...
    auto A_dense = gko::as<gko::matrix::Dense<double>>(A.get());
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            A_eigen(i, j) = A_dense->at(i, j);
        }
    }

    Eigen::JacobiSVD<Eigen::MatrixXd> svd(A_eigen);
    double cond_number = svd.singularValues()(0) / svd.singularValues().tail(1)(0);

    std::cout << "Condition number (via SVD): " << cond_number << std::endl;
    ///////////////////////////////////////////////
